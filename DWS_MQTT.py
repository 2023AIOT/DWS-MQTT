...
class ARPSManager:
    def __init__(self):
        self.alpha = 0.229  # 连接度权重
        self.beta = 0.175  # 计算能力权重
        self.gamma = 0.171  # 延迟权重
        self.delta = 0.250  # 可靠性权重
        self.lambda_ = 0.175  # 功耗权重
...
class SPFManager:
    ...
    def initialize_subscriptions(self, brokers, root_broker, stp_tree):
        ...
        修复：支持动态订阅，确保所有 broker 都能接收测试消息
        测试主题格式: test/{algorithm}/data/{i}/{j}
        """
        print("\n=== 初始化订阅 ===")
        for broker in brokers:
            if broker.id == root_broker.id:
                ...
                # 所有非根节点也订阅通配符主题，确保能接收测试消息
                # 测试主题是 test/+/data/+/+ 格式
                broker.subscription("test/#", {}, stp_tree)
                broker.subscription("test/+/data/#", {}, stp_tree)
                ...
                if depth == 1:  # 直接子节点
                    ...
                    print(f"直接子节点 {broker.id} (深度={depth}) 订阅: test/#, important/#, sensor/#, status/#")
                elif depth > 1:  # 更深层的节点
                    broker.subscription("critical/#", {}, stp_tree)
                    broker.subscription("sensor/data", {}, stp_tree)
                    broker.subscription("control/device", {}, stp_tree)
                    print(f"深层节点 {broker.id} (深度={depth}) 订阅: test/#, critical/#, sensor/data, control/device")
                else:  # 无法确定深度
                    broker.subscription("important/#", {}, stp_tree)
                    broker.subscription("sensor/#", {}, stp_tree)
                    print(f"节点 {broker.id} (深度={depth}) 订阅: test/#, important/#, sensor/#")
